...
The search script lives inside this skill's own directory, not the project directory. Invoke it by its path relative to the repository root (run it from the repository root, or substitute an absolute path):
